...
import { 
  ...
  ChevronDown, Brush, Brain, Disc, Mic2, FileMusic, ListMusic, PenTool
} from 'lucide-react';
...
const FIXED_BASE_URL = 'https://ai.danzhichen.com';
...
const MUSIC_STYLES = [
  { label: "古典", value: "Classical" }, { label: "流行", value: "Pop" }, { label: "摇滚", value: "Rock" }, 
  { label: "爵士", value: "Jazz" }, { label: "民谣", value: "Folk" }, { label: "电子", value: "Electronic" }, 
  { label: "嘻哈", value: "Hip Hop" }, { label: "蓝调", value: "Blues" }, { label: "乡村", value: "Country" }, 
  { label: "金属", value: "Metal" }, { label: "朋克", value: "Punk" }, { label: "雷鬼", value: "Reggae" }, 
  { label: "拉丁", value: "Latin" }, { label: "世界", value: "World" }, { label: "民谣摇滚", value: "Folk Rock" }, 
  { label: "说唱金属", value: "Rap Metal" }, { label: "交响金属", value: "Symphonic Metal" }, { label: "独立", value: "Indie" }, 
  { label: "实验", value: "Experimental" }, { label: "新世纪", value: "New Age" }
];
...
const PRICE_DATA = [
  ...
  },
  {
    category: '音乐创作',
    items: [
        { m: 'Suno V5 Music', p: '按次计费（请查看官方价格）' },
        { m: 'Suno V3 Lyrics', p: '按次计费' }
    ]
  }
];
...
const App = () => {
  ...
  // Music State
  const [musicTab, setMusicTab] = useState<'inspiration' | 'custom' | 'lyrics'>('inspiration');
  ...
  useEffect(() => {
    getAllAssetsFromDB().then(assets => {
        ...
        // Suno Polling
        sorted.filter(a => (a.type === 'music' || a.modelId === 'suno-v3') && (a.status === 'queued' || a.status === 'processing' || a.status === 'loading'))
              .forEach(m => startSunoPolling(m.taskId!, m.id, m.timestamp, m.type === 'music' ? 'music' : 'lyrics')); // Assume music by default for old assets
  ...
  const startSunoPolling = (taskId: string, assetId: string, startTime: number, type: 'music' | 'lyrics') => {
    const interval = setInterval(async () => {
        let key = (configRef.current.apiKey || safeEnvKey).trim();
        if (!key || !taskId) { clearInterval(interval); return; }
        try {
            const url = `${configRef.current.baseUrl}/suno/fetch/${taskId}`;
            ...
            try {
                ...
                console.warn("Polling response was not JSON", text.substring(0, 100));
                return;
            }
            
            // Standard proxy response structure:
            // data.data could be the status object OR the parent object could be it.
            // Adjusting based on standard proxy patterns observed in similar APIs:
            ...
            if (status === 'complete' || status === 'completed' || status === 'success') {
                 if (type === 'music') {
                     ...
                     const imageUrl = result.image_url || result.cover_url;
                     const title = result.title;
                     
                     if (audioUrl) {
                        const finishTime = Date.now();
                        const diff = Math.round((finishTime - startTime) / 1000);
                        const assetUpdates: Partial<GeneratedAsset> = { 
                            status: 'completed', 
                            url: audioUrl, 
                            genTimeLabel: `${diff}s`,
                            coverUrl: imageUrl,
                            title: title,
                            prompt: result.metadata?.prompt || result.prompt // Sometimes prompt is returned
                        };
                        ...
                        if (existing) {
                         ...
                         setGeneratedAssets(prev => prev.map(a => a.id === assetId ? { ...a, status: 'failed', genTimeLabel: '无音频' } : a));
                     }
                 } else if (type === 'lyrics') {
                     // Lyrics handling
                     let content = result.content || result.lyrics || result.text || result.data; // Check various fields
                     
                     // Fix for [object Object] issue: extract text if content is an object
                     if (typeof content === 'object' && content !== null) {
                        content = content.text || content.lyrics || content.content || JSON.stringify(content);
                     }

                     if (content) {
                         setMusicLyrics(String(content));
                         setMusicTab('custom');
                     }
                 ...
                 if (type === 'music') {
                     const errorMsg = result.error_message || '生成失败';
                     setGeneratedAssets(prev => prev.map(a => a.id === assetId ? { ...a, status: 'failed', genTimeLabel: errorMsg } : a));
                 } else {
                     setError('歌词生成失败: ' + (result.error_message || '未知错误'));
                     setIsMusicGenerating(false);
                 }
                 clearInterval(interval);
            }
        } catch (e) {
            console.error("Polling error for suno task", taskId, e);
        }
    }, 5000);
  };
...
只输出最终的优化后文本，不要包含任何解释。`;
  ...
  const executeSunoGeneration = async () => {
     let key = (config.apiKey || safeEnvKey).trim();
     if (!key) { setError("请先设置API Key"); return; }
     setError(null);

     // --- Case 1: Lyrics Generation ---
     if (musicTab === 'lyrics') {
         if (!prompt.trim()) { setError("请输入歌词主题或提示词"); return; }
         setIsMusicGenerating(true);
         try {
             const res = await fetch(`${config.baseUrl}/suno/submit/lyrics`, {
                 method: 'POST',
                 headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${key}` },
             ...
             try {
                ...
                console.error("Suno Lyrics API returned invalid JSON:", text);
                throw new Error(`API Endpoint Error: Server returned ${res.status} ${res.statusText}. Check network tab.`);
             }

             if (!res.ok) throw new Error(data.message || data.error || "Lyrics Request Failed");
             
             // Extract ID from response (based on provided JSON example)
             const taskId = data.data || data.id; 
             if (!taskId) throw new Error("No Task ID returned for lyrics");
             
             // Poll for result. Use a temporary placeholder ID for the polling logic if needed, 
             // but here we just need to update state, not create a gallery asset.
             startSunoPolling(taskId, 'temp-lyrics-' + Date.now(), Date.now(), 'lyrics');
     ...
     // --- Case 2: Music Generation (Custom / Inspiration) ---
     if (musicTab === 'custom') {
         if (!musicLyrics.trim()) { setError("请输入歌词"); return; }
         if (!musicStyle.trim()) { setError("请输入音乐风格"); return; }
     } else {
         if (!prompt.trim()) { setError("请输入歌曲描述"); return; }
     }
     ...
     const placeholder: GeneratedAsset = {
        id: placeholderId, url: '', type: 'music', 
        prompt: musicTab === 'custom' ? musicLyrics.slice(0, 50) + '...' : prompt.slice(0, 50) + '...',
        modelId: 'suno-v5', modelName: 'Suno V5',
        durationText: musicTab === 'custom' ? 'Custom' : 'Inspiration',
        genTimeLabel: '生成中...',
        timestamp: startTime, status: 'loading',
        config: { musicTab, musicLyrics, musicStyle, musicTitle, isInstrumental, prompt: musicTab === 'inspiration' ? prompt : '' }
     };
     ...
     try {
         ...
         if (musicTab === 'custom') {
             // Custom Generation
             endpoint = '/suno/custom_generate'; 
             body = {
                 prompt: musicLyrics,
                 tags: musicStyle,
                 title: musicTitle || "Untitled",
                 make_instrumental: false, // Explicitly false for custom mode as per requirements
                 mv: "chirp-v5"
             };
         } else {
             // Simple Generation
         ...
         try {
            data = JSON.parse(text);
             ...
             console.error("Suno Music API returned invalid JSON:", text);
             throw new Error(`API Endpoint Error: Server returned ${res.status} ${res.statusText}. Possibly wrong endpoint.`);
         }
         
         if (!res.ok) throw new Error(data.message || data.error || "Generation Request Failed");
         
         const taskId = data.id || data.data || data.task_id;
         if (!taskId) throw new Error("No Task ID returned");
         
         const updatedAsset: any = { ...placeholder, status: 'queued', taskId: taskId };
         setGeneratedAssets(prev => prev.map(a => a.id === placeholderId ? updatedAsset : a));
         saveAssetToDB(updatedAsset);
         startSunoPolling(taskId, placeholderId, startTime, 'music');
  ...
  const executeGeneration = async (overrideConfig?: any) => {
    ...
    if (isMusicMode || overrideConfig?.type === 'music') {
        executeSunoGeneration();
        return;
    }
  ...
  const handleBatchDownload = () => {
    selectedAssetIds.forEach(id => {
      ...
      if (asset && asset.url) {
        ...
        link.href = asset.url;
        link.download = `asset-${asset.id}.${asset.type === 'video' ? 'mp4' : asset.type === 'audio' ? 'wav' : asset.type === 'music' ? 'mp3' : 'png'}`;
  ...
  const handleAssetDownload = async (asset: GeneratedAsset, e: React.MouseEvent) => {
    ...
    try {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `viva-${asset.id}.${asset.type === 'video' ? 'mp4' : asset.type === 'audio' ? 'wav' : asset.type === 'music' ? 'mp3' : 'png'}`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
  ...
  const handleAssetRefresh = (asset: GeneratedAsset) => {
     if (asset.config) {
        ...
        if (asset.type === 'image') {
           ...
           setMainCategory('music');
           setMusicTab(asset.config.musicTab || 'inspiration'); // default to inspiration if old
           setMusicLyrics(asset.config.musicLyrics || '');
           setMusicStyle(asset.config.musicStyle || '');
           setMusicTitle(asset.config.musicTitle || '');
           setIsInstrumental(asset.config.isInstrumental || false);
           setPrompt(asset.config.prompt || '');
           executeSunoGeneration();
  ...
  const handleAssetGenVideo = (asset: GeneratedAsset) => { 
    if (asset.type === 'video' || asset.type === 'audio' || asset.type === 'music') return;
    setMainCategory('video'); 
    if (asset.url) {
        const matches = asset.url.match(/^data:([^;]+);base64,(.+)$/);
        const mimeType = matches ? matches[1] : 'image/png';
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-[#F1F5F9] md:h-screen overflow-hidden text-black font-sans" 
      ...
      <div className={`bg-white flex flex-col z-20 brutalist-shadow transition-all duration-300 ${isFullWidthMode ? 'flex-1 w-full border-r-0' : (isSidebarOpen ? 'w-full md:w-[450px] border-r-2 border-black' : 'w-0 md:w-0 overflow-hidden border-r-0 opacity-0')}`}>
        <header className="bg-brand-yellow pl-3 pr-5 border-b-2 border-black h-14 md:h-16 flex items-center justify-between transition-colors duration-300">
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold italic tracking-tight text-black">DZC AI助手</h1>
          </div>
          {isFullWidthMode && (
                ...
                <div className="max-w-5xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 fade-in duration-500">
                    ...
                    <div className="bg-brand-yellow border-2 border-black p-6 md:p-10 brutalist-shadow relative overflow-hidden group">
                        ...
                        <div className="relative z-10 space-y-2">
                            ...
                            <p className="text-lg font-medium opacity-80">了解 DZC AI 的最新动态与功能更新</p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white border-2 border-black p-6 brutalist-shadow-sm flex flex-col md:flex-row gap-4 items-start md:items-center relative overflow-hidden">
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="text-sm text-slate-700 font-medium">首次使用前，请务必在设置中配置您的 <a href="https://ai.danzhichen.com/console/token" target="_blank" className="bg-brand-yellow px-1 border border-black text-xs hover:opacity-80 transition-opacity">API令牌</a>，否则无法生成内容。</p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white border-2 border-black p-6 md:p-8 brutalist-shadow-sm space-y-6">
                        ...
                        <div className="space-y-4">
                             {[
                                { title: "音乐创作功能上线", desc: "接入Suno V3，支持灵感生成与专业填词模式，轻松创作属于你的音乐。" },
                ...
                <div className="max-w-5xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 fade-in duration-500">
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                            ...
                            <div className="bg-black/20 p-4 border border-white/30 backdrop-blur-sm max-w-sm">
                                <p className="text-sm font-medium leading-relaxed">
                                    加入 DZC API 合作伙伴计划，开启您的 AI 创业之旅。零门槛，高回报。
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {[
                            ...
                            "部署搭建同 ai.danzhichen.com 一样的AI聚合API平台",
                            "部署搭建同 ai.danzhichen.com 一样的AI应用平台",
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-5 pt-2 space-y-6 no-scrollbar">
          ...
          {mainCategory === 'music' ? (
              ...
              <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                  <div className="bg-brand-purple/10 border-2 border-brand-purple p-6 text-center space-y-2 brutalist-shadow-sm">
                      <h2 className="text-3xl font-black italic uppercase tracking-tighter text-brand-purple">DZC 音乐工坊</h2>
                      <p className="text-slate-600 font-normal">把任何想法都变成一首歌</p>
                  </div>

                  {/* Music Mode Tabs */}
                  <div className="flex border-2 border-black bg-white brutalist-shadow-sm">
                      {[
                          { id: 'inspiration', label: '灵感模式', sub: 'Inspiration', icon: Sparkles },
                          { id: 'custom', label: '专业模式', sub: 'Custom', icon: ListMusic },
                          { id: 'lyrics', label: '歌词创作', sub: 'Lyrics Only', icon: PenTool },
                      ].map(mode => (
                          <button
                              key={mode.id}
                              onClick={() => { 
                                  setMusicTab(mode.id as any); 
                                  resetInputState();
                                  if (mode.id === 'custom') setIsInstrumental(false); 
                              }}
                              className={`flex-1 py-3 flex flex-col items-center justify-center gap-1 transition-all border-r border-black last:border-r-0
                                  ${musicTab === mode.id 
                                      ? 'bg-brand-yellow text-black' 
                                      : 'bg-white hover:bg-slate-50 text-slate-500 hover:text-black'
                                  }`}
                          >
                              <mode.icon className="w-5 h-5" />
                              <div className="flex flex-col leading-none">
                                  <span className="text-sm font-normal uppercase">{mode.label}</span>
                                  <span className="text-[10px] font-normal uppercase">{mode.sub}</span>
                              </div>
                          </button>
                      ))}
                  </div>
                  ...
                  <div className="space-y-4">
                      ...
                      {musicTab === 'lyrics' && (
                          <div className="animate-in fade-in">
                              <div className="bg-white border-2 border-black brutalist-shadow-sm p-4 space-y-2">
                                  <label className="font-normal text-sm uppercase flex items-center gap-2">
                                      <Mic2 className="w-4 h-4 text-brand-purple" /> 歌词主题 (Topic)
                                  </label>
                                  <textarea 
                                      value={prompt}
                                      onChange={(e) => setPrompt(e.target.value)}
                                      placeholder="描述你想写的歌词主题，例如：一首关于在雨中漫步的悲伤情歌..."
                                      className="w-full h-32 p-3 border border-black outline-none text-sm resize-none bg-slate-50 focus:bg-white transition-colors"
                                  />
                              </div>
                          </div>
                      )}

                      {musicTab === 'inspiration' && (
                          <div className="bg-white border-2 border-black brutalist-shadow-sm overflow-hidden animate-in fade-in">
                              <div className="bg-slate-50 border-b border-black p-3 flex justify-between items-center">
                                  <div className="flex items-center gap-2">
                                      <Sparkles className="w-5 h-5 text-brand-purple" />
                                      <h3 className="font-normal text-sm uppercase">歌曲描述 (PROMPT)</h3>
                                  </div>
                              </div>
                              <div className="p-4 space-y-3">
                                  <textarea 
                                      value={prompt}
                                      onChange={(e) => setPrompt(e.target.value)}
                                      placeholder="描述你想要的音乐风格和内容，例如：一首快节奏的电子舞曲，女声..."
                                      className="w-full h-32 p-3 border border-black bg-white focus:bg-brand-cream/50 outline-none text-sm font-normal resize-none leading-relaxed"
                                  />
                                  <label className="flex items-center gap-2 cursor-pointer select-none border border-black p-2 w-fit bg-slate-50 hover:bg-white transition-colors">
                                      <input type="checkbox" checked={isInstrumental} onChange={e => setIsInstrumental(e.target.checked)} className="w-4 h-4 accent-brand-purple" />
                                      <span className="text-xs font-normal uppercase">纯伴奏 (Instrumental)</span>
                                  </label>
                              </div>
                          </div>
                      )}
                      ...
                      {musicTab === 'custom' && (
                          <div className="space-y-4 animate-in fade-in">
                              {/* Lyrics Module */}
                              <div className="bg-white border-2 border-black brutalist-shadow-sm overflow-hidden">
                                  <div className="bg-slate-50 border-b border-black p-3 flex justify-between items-center">
                                      <div className="flex items-center gap-2">
                                          <FileMusic className="w-5 h-5 text-brand-purple" />
                                          <h3 className="font-normal text-sm uppercase">歌词 (LYRICS)</h3>
                                      </div>
                                  </div>
                                  ...
                                  <div className="p-4 space-y-3">
                                      <textarea 
                                          ...
                                          placeholder="在这里输入歌词..."
                                          className="w-full h-48 p-3 border border-black bg-white focus:bg-brand-cream/50 outline-none text-sm font-normal resize-none whitespace-pre-wrap leading-relaxed"
                                      />
                                      <div className="flex justify-between items-center">
                                          <label className="flex items-center gap-2 cursor-not-allowed select-none border border-black p-2 w-fit bg-slate-100 opacity-50">
                                              <input type="checkbox" checked={false} disabled={true} className="w-4 h-4 accent-brand-purple" />
                                              <span className="text-xs font-normal uppercase text-slate-500">纯伴奏 (不可选)</span>
                                          </label>
                                      </div>
                                  </div>
                              </div>
                              ...
                              <div className="bg-white border-2 border-black brutalist-shadow-sm overflow-hidden">
                                  <div className="bg-slate-50 border-b border-black p-3 flex justify-between items-center">
                                      <div className="flex items-center gap-2">
                                          ...
                                          <h3 className="font-normal text-sm uppercase">风格与标题 (STYLE & TITLE)</h3>
                                      </div>
                                  </div>
                                  
                                  <div className="p-4 space-y-4">
                                      <div className="space-y-1">
                                          <label className="text-xs font-normal uppercase block">歌曲标题 (TITLE)</label>
                                          <input 
                                              value={musicTitle}
                                              onChange={e => setMusicTitle(e.target.value)}
                                              placeholder="给你的歌起个名字 (可选)..."
                                              className="w-full h-10 px-3 border border-black outline-none text-sm focus:bg-brand-cream/50"
                                          />
                                      </div>

                                      <div className="space-y-1">
                                          <label className="text-xs font-normal uppercase block">音乐风格 (STYLE)</label>
                                          <input 
                                              value={musicStyle}
                                              onChange={e => setMusicStyle(e.target.value)}
                                              placeholder="输入风格标签 (如: Pop, Rock, Sad...)"
                                              className="w-full h-10 px-3 border border-black outline-none text-sm focus:bg-brand-cream/50"
                                          />
                                      </div>
                                      
                                      <div className="relative mt-2">
                                          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2 max-h-40 overflow-y-auto pr-1 custom-scrollbar bg-slate-50 p-2 border border-black/10">
                                              {MUSIC_STYLES.map(styleObj => (
                                                  <button 
                                                      key={styleObj.value}
                                                      onClick={() => {
                                                          const current = musicStyle.split(',').map(s => s.trim()).filter(s => s);
                                                          if (!current.includes(styleObj.value)) {
                                                              setMusicStyle([...current, styleObj.value].join(', '));
                                                          }
                                                      }}
                                                      className="px-2 py-1.5 border border-black bg-white hover:bg-brand-yellow hover:-translate-y-0.5 transition-all text-[10px] md:text-xs font-normal uppercase truncate brutalist-shadow-sm text-center"
                                                      title={styleObj.value}
                                                  >
                                                      {styleObj.label}
                                                  </button>
                                              ))}
                  ...
                  {/* Music Action Button */}
                  <button 
                      onClick={() => executeSunoGeneration()} 
                      disabled={isMusicGenerating}
                      className={`w-full py-3 text-white text-xl font-normal border border-black brutalist-shadow transition-all uppercase tracking-tighter flex items-center justify-center gap-2
                          ${isMusicGenerating ? 'bg-slate-400 cursor-not-allowed' : 'bg-brand-purple hover:translate-y-1.5 hover:shadow-none'}
                      `}
                  >
                      {isMusicGenerating ? (
                          <><Loader2 className="w-5 h-5 animate-spin" /> 处理中...</>
                      ) : (
                          musicTab === 'lyrics' ? '生成歌词 / GENERATE LYRICS' : '开始创作 / CREATE MUSIC'
                      )}
                  </button>
          ...
          <div className="space-y-3">
            {!isChatMode && !isAnnouncementMode && !isProxyMode && !isMusicMode && (
              <>
            ...
            {generatedAssets.map((asset) => (
              <div key={asset.id} 
                ...
                <div className="aspect-square bg-slate-100 border-b-2 border-black relative overflow-hidden">
                  ...
                  <div className={`absolute top-3 left-3 ${asset.status === 'failed' ? 'bg-black text-white' : asset.type === 'video' ? 'bg-brand-red text-white' : (asset.type === 'audio' || asset.type === 'music') ? 'bg-brand-purple text-white' : 'bg-brand-yellow'} border border-black px-2 py-0.5 font-normal text-xs uppercase z-10`}>{asset.type}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="w-[550px] bg-brand-cream border-2 border-black brutalist-shadow animate-in zoom-in-95 relative">
            ...
            <div className="p-8 space-y-6">
              {[
                { n: '1', t: '注册与令牌', d: <>
                  前往主站 <a href="https://ai.danzhichen.com" target="_blank" className="text-blue-600 font-medium underline italic">ai.danzhichen.com</a> 注册并创建您的专属令牌。
      ...
      {/* Preview Modals */}
      {previewAsset && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4" onClick={() => setPreviewAsset(null)}>
            ...
            <div className="max-w-[90vw] max-h-[90vh] relative" onClick={e => e.stopPropagation()}>
                {previewAsset.type === 'image' ? (
                    ...
                    <video src={previewAsset.url} className="max-w-full max-h-[85vh] object-contain border-2 border-white shadow-2xl" controls autoPlay loop />
                )}
                <div className="mt-4 text-center">
                    <p className="text-white text-lg font-medium bg-black/50 px-4 py-2 rounded-full inline-block backdrop-blur-sm">{previewAsset.prompt}</p>
                </div>
            </div>
        </div>
      )}
